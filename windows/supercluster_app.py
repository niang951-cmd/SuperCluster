...
MAX_WORKERS = 200 # Higher capacity for 100+ nodes
...
class SuperClusterApp:
    def __init__(self, root):
        self.root = root
        self.root.title("🚀 SuperCluster AI - ULTRA-SCALE CONTROLLER (100+ Nodes)")
    ...
    def setup_ui(self):
        header = tk.Frame(self.root, bg='#2d2d44', height=70)
        header.pack(fill=tk.X)
        tk.Label(header, text="🧠 SuperCluster AI - Massively Distributed Node Controller", font=('Segoe UI', 16, 'bold'), fg='#89b4fa', bg='#2d2d44').pack(side=tk.LEFT, padx=20)
        ...
        main_panel = tk.Frame(self.root, bg='#1e1e2e')
        main_panel.pack(fill=tk.BOTH, expand=True, padx=10, pady=10)

        # Left Column: Chat
        left_col = tk.Frame(main_panel, bg='#1e1e2e')
        left_col.place(relx=0, rely=0, relwidth=0.55, relheight=1.0)
        ...
        tk.Button(input_frame, text="🚀 DISTRIBUTE", bg='#89b4fa', command=self.send_prompt).pack(side=tk.RIGHT)

        # Right Column: Cluster
        right_col = tk.Frame(main_panel, bg='#181825')
        right_col.place(relx=0.57, rely=0, relwidth=0.43, relheight=1.0)

        stats_frame = tk.Frame(right_col, bg='#2d2d44', pady=10)
        stats_frame.pack(fill=tk.X)
        self.node_count_label = tk.Label(stats_frame, text="NODES: 0", font=('Segoe UI', 12, 'bold'), fg='#a6e3a1', bg='#2d2d44')
        self.node_count_label.pack(side=tk.LEFT, padx=15)
        self.total_ram_label = tk.Label(stats_frame, text="RAM: 0 GB", font=('Segoe UI', 12, 'bold'), fg='#f9e2af', bg='#2d2d44')
        self.total_ram_label.pack(side=tk.RIGHT, padx=15)

        self.tree = ttk.Treeview(right_col, columns=('ip', 'load', 'ram', 'model'), show='headings')
        self.tree.heading('ip', text='Node IP')
        self.tree.heading('load', text='Load')
        self.tree.heading('ram', text='RAM')
        self.tree.heading('model', text='Model')
        self.tree.column('ip', width=120)
        self.tree.column('load', width=60)
        self.tree.column('ram', width=80)
        self.tree.column('model', width=100)
        self.tree.pack(fill=tk.BOTH, expand=True, padx=5, pady=5)

        ctrl = tk.Frame(right_col, bg='#181825', pady=5)
        ctrl.pack(fill=tk.X)
        ...
        tk.Button(ctrl, text="💾 LOAD MODEL TO RAM", bg='#f9e2af', command=self.load_model_to_all).pack(side=tk.LEFT, fill=tk.X, expand=True, padx=2)
    ...
    def discover_nodes(self):
        ...
        self.status_var.set("🚀 Massive network scan in progress...")
    ...
    def _run_massive_scan(self):
        ...
        t1 = threading.Thread(target=udp_task, args=(MULTICAST_IP,))
        t2 = threading.Thread(target=udp_task, args=('255.255.255.255', True))
        t1.start(); t2.start(); t1.join(); t2.join()

        # Parallel Subnet TCP check (100% reliable for 100+ nodes)
        def check_node(ip):
            try:
        ...
        self.executor.map(check_node, [subnet + str(i) for i in range(1, 255)])
    ...
    def _ui_update(self):
        ...
        total_ram = 0
        for ip, s in self.nodes.items():
            ram = s.get('ram', 0)
            total_ram += ram
            m_status = "RAM-RESIDENT" if s.get('model_loaded') else "EMPTY"
        ...
        self.is_scanning = False
        self.status_var.set(f"✅ Cluster Healthy: {len(self.nodes)} nodes detected")

    def load_model_to_all(self):
        self.add_msg("System", "Broadcasting LOAD_MODEL command to cluster RAM...", "#f9e2af")
        for ip in self.nodes:
            self.executor.submit(self._send_load_cmd, ip)

    def _send_load_cmd(self, ip):
        try:
            with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
                s.settimeout(5); s.connect((ip, TCP_PORT))
                s.send((json.dumps({"command": "LOAD_MODEL", "size_mb": 512}) + "\n").encode())
        except: pass
    ...
    def add_msg(self, sender, text, color):
        ...
        self.chat_area.insert(tk.END, f"{sender}: ", "b")
